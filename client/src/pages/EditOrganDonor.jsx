...
const EditOrganDonor = () => {
  ...
  newErrors.organs = "Please select at least one organ";
    ...
    const getDonorData = async () => {
      try {
        ...
        setFormData({
          ...
          coordinates: {
            latitude: donor.coordinates?.latitude || null,
            longitude: donor.coordinates?.longitude || null,
          },
          consentAccepted: donor.consentAccepted || false,
        });
  
        setLocationCaptured(
          !!(donor.coordinates?.latitude && donor.coordinates?.longitude),
        );
      } catch (error) {
    ...
    // state,dist,taluka data by getdatails api and set in formdata
    useEffect(() => {
    if (formData.state) {
  ...
  }, [formData.state, formData.district, states]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await DonorService.updateDonor(id,formData);
      setSuccess(true);
    ...
    >
      <div style={{ textAlign: "center", marginBottom: "1rem" }}>
        ...
        <p style={{ fontSize: "1.1rem", color: "var(--text-secondary)" }}>
            Update your organ donor information below.
        </p>
      </div>
  ...
  {errors.organs && (
    <div className="form-error">{errors.organs}</div>
  )}
</div>
        ...
        >
          {loading ? "Updating..." : "Organ Donor Information"}
        </button>
      </form>
    </div>
  );
